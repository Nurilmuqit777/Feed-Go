<div class="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8">
            ...
            <div class="flex flex-col sm:flex-row gap-4 md:gap-6">
                ...
                <div class="flex-1 flex flex-col sm:flex-row justify-between text-start gap-4">
                    ...
                    <div class="flex sm:flex-col flex-row-reverse justify-between sm:justify-between sm:items-end items-center">
                        ...
                        <button wire:click="remove({{ $cart->id }})" wire:confirm="Yakin ingin menghapus produk ini dari keranjang?" class="text-[#EAAA00] hover:text-yellow-600 transition p-1">
                        ...
                        <div class="bg-gray-200 rounded-full px-4 md:px-6 py-2 flex items-center gap-4 md:gap-6">
                            <button wire:click="decrease({{ $cart->id }})" class="text-xl leading-none hover:text-[#2D5016] transition w-6 h-6 flex items-center justify-center">
                                -
                            </button>
                            <span class="text-lg min-w-[20px] text-center">
                                {{ $cart->quantity }}
                            </span>
                            <button wire:click="increase({{ $cart->id }})" class="text-xl leading-none hover:text-[#2D5016] transition w-6 h-6 flex items-center justify-center">
                                +
                            </button>
                        </div>
                    </div>
    ...
    <div class="border-2 border-[#CDCDCD] rounded-3xl p-8 sticky top-5">
        ...
        <a
            href="{{ route('user.checkout') }}" class="flex items-center justify-center w-full mt-12 bg-[#EAAA00] hover:bg-yellow-500 text-white py-4 rounded-xl text-xl font-medium transition"
        >
